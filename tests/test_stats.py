...
def test_stats_tracking():
    ...
    resp = client.post(
        "/api/users", json={"admin_key": TEST_KEY, "key": ukey, "comment": "Stats User"}
    )
    uid = resp.json()["id"]
    ...
    resp = client.post(
        "/api/generate", json={"key": ukey, "html": html, "compress": False}
    )
    url = resp.json()["url"]
    ...
    client.post(
        "/api/apps", json={"key": ukey, "slug": "stat-app", "html": "Persistent"}
    )
...
def test_stats_view_stateless_attribution():
    ...
    client.post(
        "/api/users", json={"admin_key": TEST_KEY, "key": key_a, "comment": "A"}
    )
...
def test_stats_view_persistent_admin():
    ...
    client.post(
        "/api/apps", json={"key": TEST_KEY, "slug": "admin-stat", "html": "Admin App"}
    )
